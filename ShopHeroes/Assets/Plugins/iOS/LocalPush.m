...
- (void)addLocalNotice :(NSString *)title :(NSString *)subTitle :(NSString *)body :(NSNumber *)badge :(double)secs :(NSString *)identifier
{
    if (@available(iOS 10.0, *))
    {
        ...
        // 标题
        content.title = title;//@"测试标题";
        content.subtitle = subTitle;//@"测试通知副标题";
        // 内容
        ...
        NSTimeInterval time = [[NSDate dateWithTimeIntervalSince1970:secs] timeIntervalSinceNow];
    ...
    else
    {
        ...
        // 发出推送的日期
        notif.fireDate = [NSDate dateWithTimeIntervalSince1970:secs];
        // 推送的内容
        notif.alertBody = body; //@"你已经10秒没出现了";
        // 可以添加特定信息
        notif.userInfo = @{@"noticeId":identifier};
        // 角标
...
//check push notification enable
