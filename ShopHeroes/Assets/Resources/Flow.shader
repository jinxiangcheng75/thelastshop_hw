    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float2 di = float2(0.5, 0.5); // 旋转的中心位置
                uv = mul(uv, float3x3(cos(rotate), -sin(rotate), 0, sin(rotate), cos(rotate), 0, 0, 0, 1)) + di;
                uv.x += _FlowSpeed *  _Time.y;

				fixed4 flow = _FlowColor * tex2D(_FlowTex, uv) * IN.texcoord.r;
                flow.rgb = flow.rgb*_FlowColor.a;
                flow.a = color.a;
				color += flow;
